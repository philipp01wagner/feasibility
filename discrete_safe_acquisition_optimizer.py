...
from utils import sample_uniform_in_box
...
class DiscreteSafeAcquisitionOptimizer:
    def __init__(self, gp_model, safe_seed, acquisition_function, domain, sample_size, constraint_model=None):
        ...
        self._domain = domain
        # Handle both base_kernel (ScaleKernel) and direct kernel (RBFKernel)
        if hasattr(gp_model.covar_module, 'base_kernel'):
            lengthscale = gp_model.covar_module.base_kernel.lengthscale
        else:
            lengthscale = gp_model.covar_module.lengthscale

        # Handle multi-dimensional lengthscale (ARD kernel)
        if lengthscale.numel() > 1:
            self._relevant_lengthscale = lengthscale.mean().item()  # Use mean of all lengthscales
        else:
            self._relevant_lengthscale = lengthscale.item()
    ...
    def optimize(self, evaluation_points=None):
        ...
        # Return single best point with batch dimension [1, d] instead of reshaping to safe_seed shape
        if index_of_max_value >= 0:
            argmax = sample_points[index_of_max_value].unsqueeze(0)  # Shape: [1, d]
        else:
            # If no safe point found, return first safe seed point
            argmax = self._safe_seed[0:1]  # Shape: [1, d]
